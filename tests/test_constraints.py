...
from sudokutoolbox import Constraint
...
class TestConstraints:
        ...
        for i, j in np.ndindex(_nyt_hard.shape):
            if _nyt_hard[i, j] != 0:
                for k in range(nyth_const.N):
                    row_index = i * nyth_const.N + k
                    assert not nyth_const.is_safe(
                        row_index, _nyt_hard[i, j]
                    ), "Duplicate row insertion should not be allowed."
    ...
    def test_duplicate_col_insertion(self):
        ...
        for i, j in np.ndindex(_nyt_hard.shape):
            if _nyt_hard[i, j] != 0:
                for k in range(nyth_const.N):
                    col_index = k * nyth_const.N + j
                    assert not nyth_const.is_safe(
                        col_index, _nyt_hard[i, j]
                    ), "Duplicate col insertion should not be allowed."
            ...
            if _nyt_hard[i, j] != 0:
                ...
                for k in range(nyth_const.N):
                    for L in range(nyth_const.N):
                        ...
                        assert not nyth_const.is_safe(
                            m, _nyt_hard[i, j]
                        ), "Duplicate box insertion should not be allowed."

    def test_safe_to_insert_puzzle_solution(self):
        nyth_const = Constraint.from_puzzle(_nyt_hard)
        for i, j in np.ndindex(_nyt_hard.shape):
            if _nyt_hard[i, j] == 0:
                assert nyth_const.is_safe(
                    i * nyth_const.N + j, _nyth_sol[i, j]
                ), "Puzzle solution should be safe to insert."
            else:
                assert not nyth_const.is_safe(
                    i * nyth_const.N + j, _nyth_sol[i, j]
                ), "It should not be safe to doubly insert a known value."
    ...
    def test_safe_to_insert_full_solution(self):
        ...
        for i, j in np.ndindex(_nyt_hard.shape):
            if _nyt_hard[i, j] == 0:
                assert nyth_const.is_safe(
                    i * nyth_const.N + j, _nyth_sol[i, j]
                ), "Puzzle solution should be safe to insert, with constraint updating."
    ...
    def test_no_insertions_in_completed_puzzle(self):
        ...
        for i, j in np.ndindex(_nyt_hard.shape):
            for k in range(nyth_done_const.N):
                assert not nyth_done_const.is_safe(
                    i * nyth_done_const.N + j, k + _1
                ), "No insertions should be allowed in a completed puzzle."
                mask = np.int_(0)
        for value in range(1, nyth_done_const.N + 1):
            mask |= np.int_(1) << value
        ...
        assert all(
            nyth_done_const.box.flatten() == mask
        ), "Box mask was not completely filled."
    ...
    def test_double_add_is_a_noop(self):
        ...
        assert all(
            const.box.flatten() == const2.box.flatten()
        ), "Box mask should not change."
    ...
    def test_removal(self):
        ...
        assert (
            nyth_const != nyth_sol_const
        ), "Solution constraint should be different from the original constraint."
        for i, j in np.ndindex(_nyt_hard.shape):
            if _nyt_hard[i, j] != 0:
                assert (
                    _nyt_hard[i, j] == _nyth_sol[i, j]
                ), "Puzzle and solution should match for known values."
                const1 = copy.deepcopy(nyth_sol_const)
                assert not nyth_sol_const.is_safe(
                    i * nyth_sol_const.N + j, _nyt_hard[i, j]
                ), "It should not be safe to reinsert a known value."
                nyth_sol_const.remove(i * nyth_sol_const.N + j, _nyt_hard[i, j])
                print(nyth_sol_const)
                assert nyth_sol_const.is_safe(
                    i * nyth_sol_const.N + j, _nyth_sol[i, j]
                ), "Puzzle solution should be safe to insert after item removed."
                nyth_sol_const.update(i * nyth_sol_const.N + j, _nyth_sol[i, j])
                assert not nyth_sol_const.is_safe(
                    i * nyth_sol_const.N + j, _nyt_hard[i, j]
                ), "Backtracking restoration of constraint should allow reinsertion and detect attempted duplicate."
                print(const1)
                print(nyth_sol_const)
                assert (
                    const1 == nyth_sol_const
                ), "Restored constraint should be the same as the original constraint."
        for i, j in np.ndindex(_nyt_hard.shape):
            if _nyt_hard[i, j] == 0:
        ...
        assert nyth_sol_const == Constraint.from_puzzle(
            _nyt_hard
        ), "Solution constraint should be the same as the original constraint after removal and reinsertion of solved cells."
        assert (
            nyth_const == nyth_sol_const
        ), "Solution constraint should be the same as the original constraint after removal of solved cells."
