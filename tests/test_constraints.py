...
class TestConstraints:
        ...
        for i, j in np.ndindex(_nyt_hard.shape):
            if _nyt_hard[i, j] != 0:
                for k in range(nyth_const.N):
                    ...
                    assert not nyth_const.is_safe(row_index, _nyt_hard[i, j]), \
        ...
        for i, j in np.ndindex(_nyt_hard.shape):
            if _nyt_hard[i, j] != 0:
                for k in range(nyth_const.N):
                    ...
                    assert not nyth_const.is_safe(col_index, _nyt_hard[i, j]), \
                ...
                for k in range(nyth_const.N):
                    for L in range(nyth_const.N):
                        if (k//nyth_const.N != box[0] or L//nyth_const.N != box[1]):
                            continue
                        m = k*nyth_const.N + L
                        assert not nyth_const.is_safe(m, _nyt_hard[i, j]), \
    ...
    def test_safe_to_insert_puzzle_solution(self):
        ...
        for i, j in np.ndindex(_nyt_hard.shape):
            if _nyt_hard[i, j] == 0:
                assert nyth_const.is_safe(i*nyth_const.N + j, _nyth_sol[i, j]), \
                    "Puzzle solution should be safe to insert."
            else:
                assert not nyth_const.is_safe(i*nyth_const.N + j, _nyth_sol[i, j]), \
    ...
    def test_no_insertions_in_completed_puzzle(self):
        ...
        for i, j in np.ndindex(_nyt_hard.shape):
            for k in range(nyth_done_const.N):
                assert not nyth_done_const.is_safe(i*nyth_done_const.N + j, k+_1), \
    ...
    def test_removal(self):
        ...
        for i, j in np.ndindex(_nyt_hard.shape):
            if _nyt_hard[i, j] != 0:
                ...
                assert not nyth_sol_const.is_safe(i*nyth_sol_const.N + j, _nyt_hard[i, j]), \
                ...
                assert nyth_sol_const.is_safe(i*nyth_sol_const.N + j, _nyth_sol[i, j]), \
        ...
        for i, j in np.ndindex(_nyt_hard.shape):
            if _nyt_hard[i, j] == 0:
                ...
                assert nyth_sol_const.is_safe(i*nyth_sol_const.N + j, _nyt_hard[i, j]), \
                    "Puzzle solution should be safe to insert after item removed."
        assert nyth_sol_const == Constraint.from_puzzle(_nyt_hard), \
            "Solution constraint should be the same as the original constraint after removal and reinsertion of solved cells."
        assert nyth_const == nyth_sol_const, \
