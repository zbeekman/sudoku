...
import math
from attrs import define, field, Factory, cmp_using, setters, validators
...
@define(kw_only=True)
class Constraint:
    N: int = field(default=9, on_setattr=setters.frozen, validator=[validators.instance_of(int), validators.gt(0), valid_square]) # type: ignore
    ...
    b: tuple[tuple[int, int], ...] = field(
        init=False,
        repr=False,
        default=Factory(lambda self: tuple((self.r[i]//self._RN,self.c[i]//self._RN) for i in range(self.N**2)), takes_self=True))
    ...
    def is_safe(self, i: int, v: np.int_) -> bool:
        ...
        if ((self.row[self.r[i]] & shifted) or
            (self.col[self.c[i]] & shifted) or
...
def _convert(input: Union[str, npt.NDArray[np.int_], Tuple[int, ...], Tuple[Tuple[int, ...]], List[int], List[List[int]]]) -> npt.NDArray[np.int_]:
